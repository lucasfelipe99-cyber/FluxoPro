"""
MUDANÇAS em relação ao crud.py original:

1. [QUALIDADE] Import duplicado de TipoBaixa removido — antes havia duas linhas
   separadas de `from app.models.enums import ...` com TipoBaixa na primeira.
   Consolidado em uma única linha.

2. [DESIGN] baixa_pagar() — adicionado db.commit()/db.refresh() explícito após
   registrar_historico(), pois baixar_conta_pagar() do service não faz mais commit.
   Isso garante que baixa + histórico ficam na mesma transação.

3. [DESIGN] baixa_receber() — mesmo ajuste: chama db.commit()/db.refresh()
   após baixar_conta_receber(), que agora não faz mais commit internamente.

4. [QUALIDADE] Nota sobre extração de helpers para services: as funções
   sync_xml_fields_from_description, conta_pagar_item, cliente_from_nota e
   ensure_cliente_from_nota são lógica de negócio e deveriam estar em services/,
   mas foram mantidas aqui para minimizar o diff; marcar como candidatas a mover.
"""
...
# [FIX] Import consolidado — antes havia duas linhas separadas:
...
from app.models.enums import StatusCadastro, StatusPagar, StatusReceber, TipoBaixa, TipoCategoria
...
# TODO: mover para services/conta_pagar.py — é lógica de negócio, não de rota.
...
# TODO: mover para services/conta_pagar.py
...
def ensure_cliente_from_nota(db: Session, nota: NotaFiscalXML, empresa_id: int) -> Cliente:
    ...
    db.flush()
    registrar_historico(
        db, empresa_id, "clientes", cliente.id, "criado_por_nfe",
...
def list_categorias(empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    return db.scalars(
        select(Categoria)
        .where(Categoria.empresa_id == empresa_id)
...
def delete_categoria(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    if (
        db.scalar(select(ContaPagar.id).where(ContaPagar.categoria_id == entity_id))
        or db.scalar(select(ContaReceber.id).where(ContaReceber.categoria_id == entity_id))
    ):
...
def update_subcategoria(entity_id: int, payload: SubcategoriaUpdate, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    ...
    return update_entity(db, not_found(sub), payload.model_dump(exclude_unset=True))
...
def delete_subcategoria(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    ...
    sub = not_found(sub)
    if (
        db.scalar(select(ContaPagar.id).where(ContaPagar.subcategoria_id == entity_id))
        or db.scalar(select(ContaReceber.id).where(ContaReceber.subcategoria_id == entity_id))
    ):
...
def list_contas_pagar(
    ...
    stmt = (
        select(ContaPagar)
        .where(ContaPagar.empresa_id == empresa_id)
        .options(
    ...
    items = [
        ContaPagarListItem.model_validate(
...
def get_conta_pagar(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    conta = db.scalar(
        select(ContaPagar)
        .where(ContaPagar.id == entity_id, ContaPagar.empresa_id == empresa_id)
        .options(
            selectinload(ContaPagar.fornecedor),
            selectinload(ContaPagar.categoria),
            selectinload(ContaPagar.subcategoria),
    ...
    historico = db.scalars(
        ...
        .order_by(HistoricoAlteracao.created_at, HistoricoAlteracao.id)
    ).all()
...
def baixa_pagar(entity_id: int, payload: BaixaPayload, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    # [FIX] baixar_conta_pagar() não faz mais commit — a transação é controlada aqui,
    # garantindo que baixa + histórico ficam no mesmo commit.
...
def baixa_receber(entity_id: int, payload: BaixaPayload, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    # [FIX] baixar_conta_receber() não faz mais commit — controlado aqui.
    conta = baixar_conta_receber(db, not_found(get_entity(db, ContaReceber, entity_id, empresa_id)), payload)
    db.commit()
    db.refresh(conta)
    return conta
...
def list_notas(empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    return db.scalars(
        select(NotaFiscalXML)
        .where(NotaFiscalXML.empresa_id == empresa_id)
...
def delete_nota_fiscal(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    nota = not_found(db.scalar(
        select(NotaFiscalXML)
        .where(NotaFiscalXML.id == entity_id, NotaFiscalXML.empresa_id == empresa_id)
        .options(selectinload(NotaFiscalXML.produtos))
    ))
...
def update_nota_categorias(
    entity_id: int,
    payload: NotaCategoriasPayload,
    empresa_id: int = Depends(get_empresa_id),
    db: Session = Depends(get_db),
):
    nota = not_found(db.scalar(
    ...
    for conta in contas:
        registrar_historico(
            db, empresa_id, "contas_pagar", conta.id, "recalculado_xml",
            f"Lançamento recalculado pela categoria dos produtos da NF {nota.numero_nf or nota.id}.",
        )
    db.commit()
