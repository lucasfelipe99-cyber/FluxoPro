...
from app.models.entities import BaixaFinanceira, CaixaInicial, Categoria, ContaPagar, ContaReceber, Subcategoria
...
class CaixaInicialInput(BaseModel):
    mes: str
    valor: Decimal


def first_day_from_month(mes: str) -> date:
    try:
        year_text, month_text = mes.split("-")
        return date(int(year_text), int(month_text), 1)
    except ValueError as exc:
        raise HTTPException(422, "Mês inválido.") from exc


def saldo_inicial_do_mes(db: Session, empresa_id: int, first_day: date) -> Decimal:
    configured = db.scalar(select(CaixaInicial).where(
        CaixaInicial.empresa_id == empresa_id,
        CaixaInicial.mes == first_day,
    ))
    if configured:
        return Decimal(configured.valor or 0)
...
@router.get("/configuracoes/caixa-inicial")
def get_caixa_inicial(mes: str, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    first_day = first_day_from_month(mes)
    caixa = db.scalar(select(CaixaInicial).where(
        CaixaInicial.empresa_id == empresa_id,
        CaixaInicial.mes == first_day,
    ))
    return {
        "mes": first_day.strftime("%Y-%m"),
        "valor": Decimal(caixa.valor or 0) if caixa else Decimal("0"),
    }


@router.put("/configuracoes/caixa-inicial")
def put_caixa_inicial(payload: CaixaInicialInput, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    first_day = first_day_from_month(payload.mes)
    caixa = db.scalar(select(CaixaInicial).where(
        CaixaInicial.empresa_id == empresa_id,
        CaixaInicial.mes == first_day,
    ))
    if not caixa:
        caixa = CaixaInicial(empresa_id=empresa_id, mes=first_day, valor=payload.valor)
        db.add(caixa)
    else:
        caixa.valor = payload.valor
    db.commit()
    db.refresh(caixa)
    return {"mes": first_day.strftime("%Y-%m"), "valor": caixa.valor}
...
def get_fluxo_mensal(year: int, month: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    ...
    saldo_inicial = saldo_inicial_do_mes(db, empresa_id, first_day)
