...
def get_conta_pagar(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    ...
    baixas = db.scalars(
        ...
        .where(BaixaFinanceira.empresa_id == empresa_id, BaixaFinanceira.tipo == TipoBaixa.pagar, BaixaFinanceira.conta_pagar_id == entity_id)
        .order_by(BaixaFinanceira.data_baixa, BaixaFinanceira.id)
    ).all()
    historico = db.scalars(
        select(HistoricoAlteracao)
        .where(HistoricoAlteracao.empresa_id == empresa_id, HistoricoAlteracao.entidade == "contas_pagar", HistoricoAlteracao.entidade_id == entity_id)
...
def delete_nota_fiscal(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    ...
    contas = db.scalars(select(ContaPagar).where(ContaPagar.empresa_id == empresa_id, ContaPagar.nota_fiscal_id == entity_id)).all()
    ...
    for conta in contas:
        for baixa in db.scalars(select(BaixaFinanceira).where(BaixaFinanceira.empresa_id == empresa_id, BaixaFinanceira.tipo == TipoBaixa.pagar, BaixaFinanceira.conta_pagar_id == conta.id)).all():
            db.delete(baixa)
        for historico in db.scalars(select(HistoricoAlteracao).where(HistoricoAlteracao.empresa_id == empresa_id, HistoricoAlteracao.entidade == "contas_pagar", HistoricoAlteracao.entidade_id == conta.id)).all():
            db.delete(historico)
        db.delete(conta)
    db.delete(nota)
    db.commit()
...
def update_nota_categorias(entity_id: int, payload: NotaCategoriasPayload, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    ...
    for item in payload.produtos:
        ...
        categoria = db.scalar(select(Categoria).where(Categoria.id == item.categoria_id, Categoria.empresa_id == empresa_id, Categoria.tipo == TipoCategoria.despesa))
        subcategoria = db.scalar(select(Subcategoria).where(Subcategoria.id == item.subcategoria_id, Subcategoria.categoria_id == item.categoria_id))
...
def create_cliente_from_nota(entity_id: int, empresa_id: int = Depends(get_empresa_id), db: Session = Depends(get_db)):
    nota = not_found(db.scalar(select(NotaFiscalXML).where(NotaFiscalXML.id == entity_id, NotaFiscalXML.empresa_id == empresa_id)))
